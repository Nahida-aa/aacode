...
export const insertDmRoom = async (db: Db, data: DmRoomInsert[]) =>
	await db.insert(dmRoom).values(data).returning();

export const insertDmMember = async (db: Db, data: DmMemberInsert[]) =>
	await db.insert(dmMember).values(data).returning();
...
export const _listDmRoom = async (authId: string) => {
	const member = alias(dmMember, 'member');
	const otherMember = alias(dmMember, 'otherMember');

	const rooms = await db
		.select({
			...getTableColumns(dmRoom),
			lastMessage: {
				id: message.id,
		...
		.from(dmRoom)
		.innerJoin(dmMember, eq(dmMember.room_id, dmRoom.id))
		.leftJoin(message, and(eq(message.room_id, dmRoom.id)))
		.where(eq(dmMember.user_id, authId));

	const dmMembers = await db
		.select({
			room_id: dmMember.room_id,
		...
		.from(dmMember)
		.innerJoin(user, eq(user.id, dmMember.user_id))
		.where(and(eq(dmMember.user_id, authId)));
