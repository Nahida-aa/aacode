...
export const friendRequest = {
	select: getAuthFn.handler(async ({ context, errors }) => {
		...
		const friendRequestList = await db
          ...
          END
			...
			.leftJoin(
				userReceiver,
				eq(friendRequestTable.receiver_id, userReceiver.id),
			)
			.where(
				or(
	...
	send: authFn
		...
		.handler(async ({ context, input, errors }) => {
			...
			if (
				...
				existingPending[0].status === 'pending'
			) {
			...
			return await db.transaction(async (tx) => {
				// 插入或更新好友请求
				const [friendItem] = await _sendFriendRequest(
					tx,
					context.user.id,
					input,
				);
	...
	accept: authFn.input(acceptFriendRequestIn).handler(
